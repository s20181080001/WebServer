...
def calcFV(seq):
    ...
    return std_scale.transform(fv.reshape(1, 194))
...
def feature_result(seq):
    seq_list = formatSeq(seq).split('-')
    result_list = []
    iter = 0
    for one_seq in seq_list:
        if one_seq.isalpha():
            out = str(performPrediction(numpy.asarray(calcFV(one_seq))))
            result_list.append([one_seq, out, pos[iter]])
        iter = iter+1
    if result_list:
        return result_list
    else:
        return ['Invalid Sequence, Does not contains Glutamine']
...
def performPrediction(FV):
    ...
    yLabel = round(y1)
    if yLabel == 1:
        return 'Methylglutamine Site'
    else:
        return 'Non Methylglutamine Site'
